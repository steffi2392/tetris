-- A version of the Region module that allows 90 degree rotations
--   (modification by Scot Drysdale on 11/8/07)
-- and also handles Block shape and Shapes region
--   (by Chris Bailey-Kellogg, winter 2010)

-- This version of Region imports ShapeX rather than Shape.
-- Converted from Region.lhs in SOE

module RegionEX ( Region (Shape, Shapes, Translate, Scale, RotateL, Complement, 
...
data Region = Shape Shape		        -- primitive shape
            ...
            | RotateL Region            -- Rotate 90 degrees
...
(Shape s) `containsR` p
   = s `containsS` p
(Shapes ss) `containsR` p
  = any (`containsS` p) ss
(Translate (u,v) r) `containsR` (x,y)
   = let p = (x-u,y-v) in r `containsR` p
(Scale (u,v) r) `containsR` (x,y)
   = let p = (x/u,y/v) in r `containsR` p
(RotateL r) `containsR` (x,y)            -- Rotate in opposite direction
   = let p = (y,-x) in r `containsR` p
...
(Block (cx,cy) r _) `containsS` (x,y)
   = (cx-r <= x) && (x <= cx+r) && (cy-r <= y) && (y <= cy+r)
